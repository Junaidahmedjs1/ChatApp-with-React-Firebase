...
function Chat({ selectedUser, goBack }) {
  ...
  const [usersMap, setUsersMap] = useState({}); // Map of uid → userInfo

  useEffect(() => {
    // Fetch all users once and create a map uid → user
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-gray-50 space-y-4">
        {messages.map((msg, idx) => {
          const user = usersMap[msg.senderId] || {}; // Sender info
          const isCurrentUser = msg.senderId === auth.currentUser.uid;
          return (
            <div
              key={idx}
              className={`flex items-start gap-2 max-w-[70%] ${
                isCurrentUser ? "ml-auto flex-row-reverse" : "flex-row"
              }`}
            >
              <img
